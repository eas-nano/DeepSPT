...
data = importdata('data/analytics/analysis_output_200.mat');
cfg = data.cfg;
result = data.results;
...
cmap = distinguishable_colors(max(GT));
...
subplot(3,3,1); draw_track(track,color_idcs,[0 0 0; cmap]);
subplot(3,3,3); draw_track(track,GT,[0 0 0; cmap]);
subplot(3,3,4); draw_mlstates(color_idcs,fs,[0 0 0; cmap]);
subplot(3,3,6); draw_mlstates(color_idcs,fs,[0 0 0; cmap]);
subplot(3,3,7); draw_text(ML_states, ML_params, fs, umperpx, cmap, cfg.locerror)
subplot(3,3,8); draw_pdf(steps,ML_states,ML_params,umperpx,cmap);
subplot(3,3,9); draw_stephist(steps,ML_states,cmap);
